 ...
 * @author Mayank
 */
public class ViewReceiveShippmentDetailsJPanel extends javax.swing.JPanel {
     ...
     */
    public ViewReceiveShippmentDetailsJPanel(JPanel userProcessContainer, Enterprise selectedEnterprise, UserAccount userAccount, Network selectedNetwork, ReceiveShippmentWorkRequest request, ViewWorkQueueJPanel viewWorkQueueJPanel) {
        initComponents();
    ...
    private void addToInventoryJButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addToInventoryJButtonActionPerformed
        ...
        if (flag) {
            for (InventoryItem iii : request.getShipWorkRequest().getInventory().getInventoryItemList()) {
                ...
                String m = iii.getDrug().getManufacturerName();

                if (!(selectedEnterprise.getManufacturerList().contains(m))) {
                    selectedEnterprise.getManufacturerList().add(m);
                }
            }
        }
    ...
    private void verifyDrugJButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_verifyDrugJButtonActionPerformed
        ...
        int orderedNumberOfDrugs = request.getShipWorkRequest().getOrderDrugsWorkRequest().getOrder().getOrderItemList().size();
        int receivedNumberOfDrugs = request.getShipWorkRequest().getInventory().getInventoryItemList().size();

        if (orderedNumberOfDrugs != receivedNumberOfDrugs) {
            JOptionPane.showMessageDialog(null, "Less Number Of Drugs Received");
            JOptionPane.showMessageDialog(null, "Report Sent To FDA");
            JOptionPane.showMessageDialog(null, "Show Details");

            TheftWorkRequest req = (TheftWorkRequest) govt.getComplaintHandlingOrganization().getWorkQueue().newWorkRequest(WorkRequest.Type.TheftWorkRequest);
            req.setReceiveShippmentWorkRequest(request);
            req.setTheftInventory(request.getShipWorkRequest().getTheftInventory());
            req.setSender(userAccount);
            req.setStatus(WorkRequest.Status.THEFT);

            req.setRequestDate(d1);
            flag1 = false;
            ViewTheftDetailJPanel viewTheftDetailJPanel = new ViewTheftDetailJPanel(userProcessContainer, reqEnterprise, userAccount, selectedNetwork, request, selectedEnterprise);
            userProcessContainer.add("TheftDetail", viewTheftDetailJPanel);
            CardLayout cardLayout = (CardLayout) userProcessContainer.getLayout();
